...
function writeRows(ss, data) {
  ...
  if (!sheet) {
    ...
    const header = isS1
      ? ['timestamp','evaluator','sample_id','class_code','class_name','sample_num',
  ...
  if (isS1) {
    data.rows.forEach(r => sheet.appendRow([
      ts, ev, r.sample_id, r.class_code, r.class_name, r.sample_num,
    ...
    data.rows.forEach(r => sheet.appendRow([
      ts, ev, r.image_id, r.class_code, r.class_name, r.sample_num,
